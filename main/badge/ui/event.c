...
void ui_event_load_schedule() {
  ...
  lv_table_set_row_count(table_event, size);
  ...
  for (int i = 0; i < size; i++) {
    ...
    snprintf(buff, sizeof(buff), "%s\nby %s", title->valuestring, speaker->valuestring);
...
lv_obj_t *ui_event_init() {
  ...
  lv_table_set_column_count(table_event, 2);
  ...
  lv_table_set_column_width(table_event, 0, LV_HOR_RES / 2);
  lv_table_set_column_width(table_event, 1, LV_HOR_RES / 2);
