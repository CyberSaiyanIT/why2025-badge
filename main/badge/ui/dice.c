...
void ui_dice_set_button_color(lv_draw_task_t *draw_task, bool pressed, lv_palette_t color) {
...
static void ui_dice_draw_event(lv_event_t *e) {
  ...
  if (base_dsc->part == LV_PART_ITEMS) {
    ...
    if (base_dsc->id1 == 0)
      ui_dice_set_button_color(draw_task, pressed, LV_PALETTE_RED);
    else if (base_dsc->id1 == 1)
      ui_dice_set_button_color(draw_task, pressed, LV_PALETTE_BLUE);
    else if (base_dsc->id1 == 2)
      ui_dice_set_button_color(draw_task, pressed, LV_PALETTE_GREEN);
  }
}

static uint8_t ui_dice_random_roll() {
  return (esp_random() % dice_rolls[dice_id].faces) + 1;
}

static void ui_dice_timer_handler(lv_timer_t *timer) {
  uint8_t roll[2];
  uint8_t result = 0;

  switch (dice_rolls[dice_id].roll_mode) {
    case NORMAL_ROLL:
      result = ui_dice_random_roll(dice_id);
      lv_label_set_text_fmt(roll_label, "%d", result);
      break;
    case DISADVANTAGE_ROLL:
      roll[0] = ui_dice_random_roll(dice_id);
      roll[1] = ui_dice_random_roll(dice_id);
...
static void ui_dice_roll_event(lv_event_t *e) {
  lv_obj_t *obj = lv_event_get_target_obj(e);
  dice_id       = lv_buttonmatrix_get_selected_button(obj);

  ui_dice_show_result_win();
  ui_dice_reset_timer();
}

static void ui_dice_reroll_event(lv_event_t *e) {
  lv_timer_set_repeat_count(dice_timer_handle, NB_ROLLS);
  lv_timer_resume(dice_timer_handle);
}

static void ui_dice_close_event(lv_event_t *e) {
...
lv_obj_t *ui_dice_init() {
  ...
  lv_obj_add_event_cb(dice_btnmtx, ui_dice_roll_event, LV_EVENT_PRESSED, NULL);

  lv_obj_add_event_cb(dice_btnmtx, ui_dice_draw_event, LV_EVENT_DRAW_TASK_ADDED, NULL);
  ...
  lv_obj_add_event_cb(result_close, ui_dice_close_event, LV_EVENT_PRESSED, NULL);
  ...
  lv_obj_add_event_cb(result_label, ui_dice_reroll_event, LV_EVENT_PRESSED, NULL);
  ...
  lv_obj_add_event_cb(roll_label, ui_dice_reroll_event, LV_EVENT_PRESSED, NULL);
  ...
  lv_obj_add_event_cb(result_reroll, ui_dice_reroll_event, LV_EVENT_PRESSED, NULL);
  lv_obj_add_flag(result_reroll, LV_OBJ_FLAG_EVENT_BUBBLE);

  dice_timer_handle = lv_timer_create(ui_dice_timer_handler, 100, NULL);
