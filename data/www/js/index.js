...
var person_form = document.querySelector("#person-form");
var person_name_input = document.querySelector("#person-name");
...
async function check_authentication(key) {
    query_authentication(key).then(() => {
        query_name(key, null).then(name => {
            setInput(badge_name_input, name)
        });
        query_person(key, null, null, null, null).then(person => {
            console.log(person)
    ...
    }).then(setAuthenticated(true)).catch(() => {
        setAuthenticated(false)
    })
}
async function login(password) {
    query_login(password).then(data => {
        Toastify({
...
badge_name_form.addEventListener("submit", function (evt) {
    ...
    if (badge_name_input.value.length > 0) {
        query_name(getCookie("key"), badge_name_input.value).then(name => {
            setInput(badge_name_input, name)
        }).then(() => Toastify({
...
person_form.addEventListener("submit", function (evt) {
    evt.preventDefault();
    ...
    if (person_name_input.value.length > 0) {
        query_person(getCookie("key"), person_name_input.value, person_organization_input.value,
            person_job_input.value, person_message_input.value,  person_url_input.value).then(person => {
                ...
                text: "Error. Cannot change person infomation.",
                duration: 3e3,
                position: "center",
                style: {
                    background: "#dc3545"
...
wifi_ssid_form.addEventListener("submit", function (evt) {
    ...
    if (wifi_ssid_input.value.length > 0) {
        query_wifi(getCookie("key"), wifi_ssid_input.value, null, null).then(wifi => {
            setInput(wifi_ssid_input, wifi.ssid)
        }).then(() => Toastify({
...
web_password_form.addEventListener("submit", function (evt) {
    ...
    if (web_password_input1.value.length > 0 && web_password_input2.value.length > 0) {
        if (web_password_input1.value === web_password_input2.value) {
            query_password(getCookie("key"), web_password_input1.value).then(data => {
                logout(getCookie("key"));
...
wifi_password_form.addEventListener("submit", function (evt) {
    ...
    if (wifi_password_input1.value.length > 0 && wifi_password_input2.value.length > 0) {
        if (wifi_password_input1.value === wifi_password_input2.value) {
            query_wifi(getCookie("key"), null, wifi_password_input1.value, null).then(() => {
...
reset_button.addEventListener("click", function (evt) {
    evt.preventDefault();
    query_reset(getCookie("key")).then(() => { }).then(() => Toastify({
